...
@implementation BaseRefreshHeadView
...
- (void)buildUI
{
    ...
    self.label.text = NSLocalizedString(Refresh_normal_title, nil);
...
- (void)setCurrentStatus:(FMRefreshState)currentStatus {
    ...
    switch (_currentStatus) {
        case FMRefreshStateNormal:
            ...
            self.label.text = Refresh_normal_title;
            [self.label sizeToFit];
        ...
        case FMRefreshStatePulling:
            NSLog(@"切换到Pulling");
            self.label.text = Refresh_pulling_title;
            [self.label sizeToFit];
        ...
        case FMRefreshStateRefreshing:
            NSLog(@"切换到Refreshing");
            self.label.text = Refresh_Refreshing_title;
            [self.label sizeToFit];
...
- (void)endRefreshing {
    ...
    NSLog(@"213---------------:endRefreshing");
    if (self.currentStatus != FMRefreshStateRefreshing) {
        return;
    }
    
    NSLog(@"218---------------:endRefreshing");
    self.currentStatus = FMRefreshStateNormal;
    [super endRefreshing];
